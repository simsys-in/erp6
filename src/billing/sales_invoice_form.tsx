...
export class Sales_invoice_form extends React.Component  <any, SomeState> {
  ...
  componentDidUpdate(prevProps:any, prevState:any) {
    ...
    if (this.props.match.path === '/billing/sales_invoice/print/:uid') {
      //v ={uid: props.match.params.uid, action: 'preview' }
    } else if (this.props.match.path === '/billing/sales_invoice/:uid' && (this.state.action != 'edit')) {
      this.view(this.props.match.params.uid)
    } else if (this.props.match.path === '/billing/sales_invoice' && this.state.action != 'new' ) {
      this.new()
    } else if (this.props.match.path === '/billing/sales_invoice/list' && this.state.action != 'list' ) {
      this.list()
    }else{
  ...
  componentDidMount() {
    ...
    if (this.props.match.path === '/billing/sales_invoice/print/:uid') {
      //v ={uid: props.match.params.uid, action: 'preview' }
    } else if (this.props.match.path === '/billing/sales_invoice/:uid') {
      this.view(this.props.match.params.uid)
    } else if (this.props.match.path === '/billing/sales_invoice/list') {
  ...
  save = () => {
    ...
    axios.post('billing/sales_invoice',this.state.f)
    .then(r=>{
      console.log(r);
  ...
  view = (uid: String) => {
    ...
    axios.get('billing/sales_invoice/'+uid).then(r=>{
  ...
  render() {
    return (
      <Container className="mt-2">
        <Card>
          <Card.Header as="h5" className="d-print-none"><Icons.CartPlusFill /> Sales Invoice <Badge bg="info">{this.state.action}</Badge>
            <div className="float-end">
              <ButtonToolbar aria-label="Toolbar with Button groups">
                <ButtonGroup aria-label="First group" className=" mt-1">
                <Link to={`/billing/sales_invoice/list`}><Icons.ListUl className={"text-primary mx-2 " + (this.state.action === 'new' || this.state.action === 'edit' || this.state.action === 'preview' ? '' : 'd-none')}  size="1em" /></Link>
                  ...
                  <Link to={`/billing/sales_invoice`}><Icons.Files className={"text-success mx-2 " + (this.state.action === 'list' || this.state.action === 'preview' ? '' : 'd-none')} style={{ cursor: "hand" }} size="1em" /></Link>
          ...
          <Card.Body className={this.state.action === 'new' || this.state.action === 'edit' ? '' : "d-none"}>
            ...
            <Row>
              <Col md={4}><InputGroup className="mb-1 input-group-sm"><InputGroup.Text>Ledger</InputGroup.Text><FormControl className="fw-bold" id="ledger_category" onChange={this.setd} value={this.state.f.ledger_category} /></InputGroup></Col>
            </Row>
          </Card.Body>
